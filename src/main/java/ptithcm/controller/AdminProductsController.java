...
//		pd : product detail
	...
	public String addProduct(@ModelAttribute("pd") ProductDetail pd,ModelMap model) {
		try {
			List<MultipartFile> files = pd.getFiles();
			Boolean res = productDao.addProductDetail(pd);
			if(!res) {
				model.addAttribute("message","Thêm chi tiết hàng thất bại");
				return "page/admin/handleProductDetail";
			}
			for (int i=0;i<files.size() ;i++) {
				...
				if (!file.isEmpty()) {
					...
					Map<String, String> m = cloudinary.uploader().upload(file.getBytes(), ObjectUtils.asMap(
							"resource_type", "auto", "folder", "WebAoQuan/Products", "public_id", file.getOriginalFilename()));
					...
					if(!productImage.addProductImage(pi)) {
						model.addAttribute("message","Thêm ảnh số "+i+" thất bại");
						return "page/admin/handleProductDetail";
					}
				}
			}
		} catch (Exception e) {
			model.addAttribute("message","Xảy ra lỗi khi thêm chi tiết sản phẩm");
			e.printStackTrace();
